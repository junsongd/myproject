...
@interface LeonAssetPickerController ()
...
@implementation LeonVideoTitleView
...
-(void)drawRect:(CGRect)rect{
    ...
    UIImage *videoIcon=[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"LeonAssetPicker.Bundle/Images/AssetsPickerVideo@2x.png"]];
...
@interface LeonTapAssetView ()

@property(nonatomic,retain)UIImageView *selectView;

@end

@implementation LeonTapAssetView
...
+ (void)initialize
{
    checkedIcon     = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"LeonAssetPicker.Bundle/Images/%@@2x.png",(!IS_IOS7) ? @"AssetsPickerChecked~iOS6" : @"AssetsPickerChecked"]]];
...
@interface LeonAssetView ()<LeonTapAssetViewDelegate>

@property (nonatomic, strong) ALAsset *asset;

@property (nonatomic, weak) id<LeonAssetViewDelegate> delegate;

@property (nonatomic, retain) UIImageView *imageView;
@property (nonatomic, retain) LeonVideoTitleView *videoTitle;
@property (nonatomic, retain) LeonTapAssetView *tapAssetView;

@end

@implementation LeonAssetView
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame])
    {
        ...
        _videoTitle=[[LeonVideoTitleView alloc] initWithFrame:CGRectMake(0, kThumbnailSize.height-20, kThumbnailSize.width, titleHeight)];
        ...
        _tapAssetView=[[LeonTapAssetView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
...
#pragma mark - LeonTapAssetView Delegate
...
@interface LeonAssetViewCell ()<LeonAssetViewDelegate>

@end

@class LeonAssetViewController;

@implementation LeonAssetViewCell
    ...
    if (self.contentView.subviews.count<assets.count) {
        for (int i=0; i<assets.count; i++) {
            if (i>((NSInteger)self.contentView.subviews.count-1)) {
                LeonAssetView *assetView=[[LeonAssetView alloc] initWithFrame:CGRectMake(assetViewX+(kThumbnailSize.width+minimumInteritemSpacing)*i, minimumLineSpacing-1, kThumbnailSize.width, kThumbnailSize.height)];
                [assetView bind:assets[i] selectionFilter:selectionFilter isSeleced:[((LeonAssetViewController*)_delegate).indexPathsForSelectedItems containsObject:assets[i]]];
                assetView.delegate=self;
                [self.contentView addSubview:assetView];
            }
            else{
                ((LeonAssetView*)self.contentView.subviews[i]).frame=CGRectMake(assetViewX+(kThumbnailSize.width+minimumInteritemSpacing)*(i), minimumLineSpacing-1, kThumbnailSize.width, kThumbnailSize.height);
                [(LeonAssetView*)self.contentView.subviews[i] bind:assets[i] selectionFilter:selectionFilter isSeleced:[((LeonAssetViewController*)_delegate).indexPathsForSelectedItems containsObject:assets[i]]];
            }
        ...
        for (int i=self.contentView.subviews.count; i>0; i--) {
            if (i>assets.count) {
                [((LeonAssetView*)self.contentView.subviews[i-1]) removeFromSuperview];
            }
            else{
                ((LeonAssetView*)self.contentView.subviews[i-1]).frame=CGRectMake(assetViewX+(kThumbnailSize.width+minimumInteritemSpacing)*(i-1), minimumLineSpacing-1, kThumbnailSize.width, kThumbnailSize.height);
                [(LeonAssetView*)self.contentView.subviews[i-1] bind:assets[i-1] selectionFilter:selectionFilter isSeleced:[((LeonAssetViewController*)_delegate).indexPathsForSelectedItems containsObject:assets[i-1]]];
            }
        }
    }
}

#pragma mark - LeonAssetView Delegate
...
@interface LeonAssetViewController ()<LeonAssetViewCellDelegate>{
    int columns;
...
@implementation LeonAssetViewController
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    LeonAssetPickerController *picker = (LeonAssetPickerController *)self.navigationController;
    
    LeonAssetViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell==nil) {
        cell=[[LeonAssetViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
    }
    cell.delegate=self;
...
#pragma mark - LeonAssetViewCell Delegate

- (BOOL)shouldSelectAsset:(ALAsset *)asset
{
    LeonAssetPickerController *vc = (LeonAssetPickerController *)self.navigationController;
...
@interface LeonAssetGroupViewCell ()

@property (nonatomic, strong) ALAssetsGroup *assetsGroup;

@end

@implementation LeonAssetGroupViewCell
...
@interface LeonAssetGroupViewController()
...
@implementation LeonAssetGroupViewController
...
- (void)showNotAllowed
{
    ...
    UIImageView *padlock    = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"LeonAssetPicker.Bundle/Images/AssetsPickerLocked@2x.png"]]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    LeonAssetGroupViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell == nil)
    {
        cell = [[LeonAssetGroupViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:CellIdentifier];
    }
...
{
    LeonAssetViewController *vc = [[LeonAssetViewController alloc] init];
...
@implementation LeonAssetPickerController

- (id)init
{
    LeonAssetGroupViewController *groupViewController = [[LeonAssetGroupViewController alloc] init];
